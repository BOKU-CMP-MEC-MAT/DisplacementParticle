...
namespace Marmot::Meshfree {
   ...
   *        and Naturally Stabilized Nodal Integration (NSNI) using only the deviatoric part of the stress tensor.
   *
   * This variant uses only the deviatoric stress (dev(S)) in the NSNI stabilization, filtering out the
   * hydrostatic/volumetric component. This is useful for applications where volumetric effects should be
   * treated separately or where material models are primarily sensitive to deviatoric stress.
   *
  ...
  class DisplacementParticleSQCNIxNSNIDev : public DisplacementParticleSQCNI< nDim, nVertices > {
  ...
  public:
    /**
     * @brief Constructor for DisplacementParticleSQCNIxNSNIDev.
     *
     ...
     */
    DisplacementParticleSQCNIxNSNIDev( int                                            elementID,
     ...
     *
     * This method implements the core physics computations for the NSNI particle using only
     * the deviatoric part of the stress tensor. It calculates the internal forces (`fInt`) 
     * and the tangent stiffness matrix (`dFInt_ddQ`) based on the current deformation, 
     * material response, and the NSNI stabilization terms.
     *
  ...
  template < int nDim, int nVertices >
  DisplacementParticleSQCNIxNSNIDev< nDim, nVertices >::DisplacementParticleSQCNIxNSNIDev(
    int                                                  elementID,
  ...
  template < int nDim, int nVertices >
  void DisplacementParticleSQCNIxNSNIDev< nDim, nVertices >::computePhysicsKernels( const double* dQ,
                                                                                    double*       fInt,
                                                                                    double*       dFInt_ddQ,
                                                                                    double        timeNew,
                                                                                    double        dT )
  {
    ...
    // Compute deviatoric part of dS_dY for use in NSNI stabilization
    TensorDDD dS_dev_dY( 0.0 );
    for ( int K = 0; K < nDim; K++ ) {
      double traceDSK = 0.0;
      for ( int i = 0; i < nDim; i++ ) {
        traceDSK += dS_dY( i, i, K );
      }
      for ( int i = 0; i < nDim; i++ ) {
        for ( int j = 0; j < nDim; j++ ) {
          dS_dev_dY( i, j, K ) = dS_dY( i, j, K );
          if ( i == j ) {
            dS_dev_dY( i, j, K ) -= ( traceDSK / 3.0 );
          }
        }
      }
    }
    ...
    for ( int A = 0; A < _nNodes; A++ ) {
      ...
      // Use full stress in the base term
      ...
      // Use deviatoric stress gradient in the NSNI stabilization term
      TensorD rU_Stab = einsum< iK, ijK >( d2NA_dxdY_x_MOIScaled, dS_dev_dY );
      ...
      for ( int B = 0; B < _nNodes; B++ ) {
        ...
        // Use full stress in geometric stiffness term
        ...
        // Apply deviatoric projection to the stiffness tensor to maintain consistency
        TensorDDDD d2S_dev_dqu_dY( 0.0 );
        for ( int k = 0; k < nDim; k++ ) {
          for ( int K = 0; K < nDim; K++ ) {
            double traceDSK = 0.0;
            for ( int i = 0; i < nDim; i++ ) {
              traceDSK += d2S_dqu_dY( i, i, k, K );
            }
            for ( int i = 0; i < nDim; i++ ) {
              for ( int j = 0; j < nDim; j++ ) {
                d2S_dev_dqu_dY( i, j, k, K ) = d2S_dqu_dY( i, j, k, K );
                if ( i == j ) {
                  d2S_dev_dqu_dY( i, j, k, K ) -= ( traceDSK / 3.0 );
                }
              }
            }
          }
        ...
        const TensorDD dNB_dx_times_dS_dev_dY = einsum< ijK, i >( dS_dev_dY, dN_B_dx );

        const TensorDD dRU_Stab_dU_1   = einsum< iK, ijkK, to_jk >( d2NA_dxdY_x_MOIScaled, d2S_dev_dqu_dY );
        const TensorDD dRU_Stab_x_dU_2 = -einsum< kK, jK, to_jk >( d2NA_dxdY_x_MOIScaled, dNB_dx_times_dS_dev_dY );
